...
const (
	...
	// TODO: support yanked releases
	// VersionTitleYankedRegex                string = `^## \[(?P<version>` + SemVerRegex + `)\] - (?P<date>` + DateRegex + `) [YANKED]$`
	// VersionTitleYankedWithLinkRegex        string = `^## \[(?P<version>` + SemVerRegex + `)\]\((?P<url>` + URLRegex + `)\) - (?P<date>` + DateRegex + `) [YANKED]$`
